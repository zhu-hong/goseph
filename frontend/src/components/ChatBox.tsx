...
export function ChatBox() {
  ...
  return  <div className="flex-none flex justify-between items-center dark:text-white">
    <form
      onSubmit={onSubmit}
      className="flex-auto rounded-xl overflow-hidden h-48px input-form flex justify-between items-center dark:bg-dark-700"
    >
      <input
        value={text}
        ...
        placeholder="。。。"
        type="text"
        className="flex-auto bg-transparent h-full pl-4 nodrag focus:border-none focus:outline-none" 
      />
      <button type="submit" className={['flex-none w-38px h-38px rounded-full grid place-items-center transition  mx-1', text.length === 0 ? 'text-gray-400 cursor-not-allowed' : 'text-rose-400 active:bg-light-100 dark:active:bg-dark-100'].join(' ')}>
    ...
    <button className="flex-none w-48px h-48px ml-2 rounded-full grid place-items-center text-gray-400 transition active:bg-light-100 dark:active:bg-dark-100">
